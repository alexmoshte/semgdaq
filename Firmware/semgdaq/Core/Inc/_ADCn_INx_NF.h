 ...
 *
 * This header file consists of all the variables and pointers that are to be fed to the CMSIS DSP functions for an IIR biquad filter implementation
...
#include "arm_math.h" // Header file necessary for running CMSIS DSP functions
...
typedef struct   // Handles IN1 channel data
{
	...
	float32_t *ADC1_IN1_psrc; // Pointer to the input/source buffer. The source buffer in this case is one that stores sorted data from channel 1 of ADC1

	float32_t _NF_ADC1_IN1_bfr[ADC_DMA_HALFBUFFERSIZE]; // Buffer that stores the filtered data

	float32_t *ADC1_IN1_pdst; // Pointer to the output/destination buffer
...
{
	uint8_t ADC1_IN2_numstages;

	float32_t  ADC1_IN2_coeffs[2*NOOFCOEFFICIENTS];

	const float32_t *ADC1_IN2_pcoeffs;

	float32_t ADC1_IN2_States[2*NOOFS_VARIABLES];

	float32_t *ADC1_IN2_pState;

	float32_t *ADC1_IN2_psrc;

	float32_t _NF_ADC1_IN2_bfr[ADC_DMA_HALFBUFFERSIZE];

	float32_t *ADC1_IN2_pdst;

	uint32_t ADC1_IN2_Blocksize;

	const arm_biquad_cascade_df2T_instance_f32 S2;
...
{
	uint8_t ADC2_IN3_numstages;

	float32_t  ADC2_IN3_coeffs[2*NOOFCOEFFICIENTS];

	const float32_t *ADC2_IN3_pcoeffs;

	float32_t ADC2_IN3_States[2*NOOFS_VARIABLES];

	float32_t *ADC2_IN3_pState;

	float32_t *ADC2_IN3_psrc;

	float32_t _NF_ADC2_IN3_bfr[ADC_DMA_HALFBUFFERSIZE];

	float32_t *ADC2_IN3_pdst;

	uint32_t ADC2_IN3_Blocksize;

	const arm_biquad_cascade_df2T_instance_f32 S3;
...
{
	uint8_t ADC2_IN4_numstages;

	float32_t  ADC2_IN4_coeffs[2*NOOFCOEFFICIENTS];

	const float32_t *ADC2_IN4_pcoeffs;

	float32_t ADC2_IN4_States[2*NOOFS_VARIABLES];

	float32_t *ADC2_IN4_pState;

	float32_t *ADC2_IN4_psrc;

	float32_t _NF_ADC2_IN4_bfr[ADC_DMA_HALFBUFFERSIZE];

	float32_t *ADC2_IN4_pdst;

	uint32_t ADC2_IN4_Blocksize;

	const arm_biquad_cascade_df2T_instance_f32 S4;
...
typedef struct // Handles IN1 channel data
{
	uint8_t ADC3_IN1_numstages;

	float32_t  ADC3_IN1_coeffs[2*NOOFCOEFFICIENTS];

	const float32_t *ADC3_IN1_pcoeffs;

	float32_t ADC3_IN1_States[2*NOOFS_VARIABLES];

	float32_t *ADC3_IN1_pState;

	float32_t *ADC3_IN1_psrc;

	float32_t _NF_ADC3_IN1_bfr[ADC_DMA_HALFBUFFERSIZE];

	float32_t *ADC3_IN1_pdst;

	uint32_t ADC3_IN1_Blocksize;

	const arm_biquad_cascade_df2T_instance_f32 S5;
...
void init_ADC1_IN1_struct(void);  // Function that initializes the pointer to the source buffer at runtime for channel 1
...
void update_ADC1_IN1_FO_biquad_filter(void);  // The optimized CMSIS DSP function that handles all the filtering operations and uploads the updated values to the destination buffers for channel 1
